...
unsafe fn deserialize_array(context: *mut JSContext, v: JSValue) -> Result<Vec<Value>, String> {
    ...
    for index in 0..(len as usize) {
        ...
        values.push(Value {
            ctx: context,
            v: value_raw,
            tag: JS_VALUE_GET_NORM_TAG_real(value_raw),
        });
    }
    Ok(values)
}
...
unsafe fn deserialize_object(
    ...
    if JS_VALUE_GET_NORM_TAG_real(obj) != JS_TAG_OBJECT {
    ...
    for index in 0..count {
        ...
        if JS_VALUE_GET_NORM_TAG_real(raw_value) == JS_TAG_EXCEPTION {
            js_std_dump_error(context);
        ...
        let value = Value {
            ctx: context,
            v: raw_value,
            tag: JS_VALUE_GET_NORM_TAG_real(raw_value),
        };

        let key_value = JS_AtomToString(context, (*prop).atom);
        if JS_VALUE_GET_NORM_TAG_real(key_value) == JS_TAG_EXCEPTION {
            js_std_dump_error(context);
...
unsafe fn to_string(ctx: *mut JSContext, r: JSValue) -> Result<String, String> {
    if JS_VALUE_GET_NORM_TAG_real(r) != JS_TAG_STRING {
...
unsafe fn set(
    ...
    if JS_VALUE_GET_NORM_TAG_real(this_obj) == JS_TAG_OBJECT {
...
impl Value {
    unsafe fn new(ctx: *mut JSContext, val: JSValue) -> Self {
        Self {
            ctx,
            v: val,
            tag: JS_VALUE_GET_NORM_TAG_real(val),
        }
    }

    pub fn set(&mut self, name: &str, v: Value) -> Result<(), String> {
        unsafe {
            if JS_VALUE_GET_NORM_TAG_real(self.v) == JS_TAG_OBJECT {
